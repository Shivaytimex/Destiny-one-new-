...
export default function MatchesPage() {
  ...
  useEffect(() => { frontendRepository.matches.list().then(items=>items.length&&setMatches(items)); }, []);
  const current = matches[0];
  async function decide(kind) {
    if (!current) return;
    await frontendRepository.matches.saveDecision(current.id,kind);setNotice("Saved in frontend preview.");
