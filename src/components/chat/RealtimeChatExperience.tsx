...
import {
  ...
  Link2, MapPin, MoreVertical, Pause, Phone, Pin, Play, RotateCcw, Search, Settings, ShieldCheck, Star, Trash2, Video, Wifi, WifiOff, X,
} from "lucide-react";
...
const gifCatalog = [
  { id: "love", emoji: "🥰", title: "Lots of love", tags: "love heart romantic thinking of you" },
  { id: "hug", emoji: "🤗", title: "Big warm hug", tags: "hug comfort miss you cozy" },
  { id: "kiss", emoji: "😘", title: "Sending a kiss", tags: "kiss love romantic cute" },
  { id: "morning", emoji: "🌅", title: "Good morning", tags: "good morning sunrise coffee" },
  { id: "laugh", emoji: "🤣", title: "I cannot stop laughing", tags: "laugh funny lol reaction" },
  { id: "sorry", emoji: "🥺", title: "I am sorry", tags: "sorry apology forgive me" },
  { id: "date", emoji: "🌹", title: "Date night", tags: "date rose romance tonight" },
  { id: "celebrate", emoji: "🎉", title: "Celebrate together", tags: "celebration congratulations party" },
];
...
export default function RealtimeChatExperience() {
  ...
  const visibleGifs = (gifTab === "recent" ? gifLibrary.recents : gifTab === "favourites" ? gifLibrary.favourites : gifCatalog).filter((gif) => !gifQuery.trim() || `${gif.title} ${gif.tags || ""}`.toLowerCase().includes(gifQuery.trim().toLowerCase()));
  ...
  return <div className="pro-chat-shell">
    ...
    <section className="pro-chat-main">
      ...
      <div className="pro-chat-messages"><div className="pro-chat-day">TODAY</div>{filteredMessages.map((message) => {
        ...
        return <div ref={(node) => { if (node) messageRefs.current[message.id] = node; }} id={`message-${message.id}`} key={message.id} className={`pro-message-row ${mine ? "mine" : "theirs"}`}><div className={`pro-message ${message.type}`}>
          {message.deletedForEveryoneAt ? <p className="pro-message-deleted">This message was deleted</p> : <>
            ...
            {message.type === "gif" && <div className="pro-message-gif" role="img" aria-label={`${message.payload?.gif?.title || message.body} animated GIF`}><span>{message.payload?.gif?.emoji || "✨"}</span><strong>{message.payload?.gif?.title || message.body}</strong><small>Animated reaction</small></div>}
            {!["date", "image", "location", "voice", "gif"].includes(message.type) && <p>{message.body}</p>}
      ...
      {attachments && <div className="pro-chat-attachments"><button onClick={() => fileInput.current?.click()}><ImageIcon />Photo</button><button onClick={() => { setGifOpen(true); setGifTab("all"); setAttachments(false); }}><ImageIcon />GIF</button><button onClick={shareLocation}><MapPin />Location</button><button onClick={() => setPanel("date")}><CalendarDays />Date plan</button><button onClick={() => send("Relationship values.pdf", { type: "document", payload: { name: "Relationship values.pdf" } })}><FileText />Document</button></div>}
      {gifOpen && <div className="pro-gif-library" role="dialog" aria-label="GIF library"><div className="pro-gif-head"><div><strong>GIF reactions</strong><small>Recents and favourites stay cached on this device.</small></div><button onClick={() => setGifOpen(false)} aria-label="Close GIF library"><X size={16} /></button></div><div className="pro-gif-tabs" role="tablist">{[["all", "All"], ["recent", "Recents"], ["favourites", "Favourites"]].map(([id, label]) => <button key={id} role="tab" aria-selected={gifTab === id} className={gifTab === id ? "active" : ""} onClick={() => setGifTab(id)}>{label}</button>)}</div><label className="pro-gif-search"><Search size={14} /><input aria-label="Search GIFs" value={gifQuery} onChange={(event) => setGifQuery(event.target.value)} placeholder="Search love, hug, morning…" /></label><div className="pro-gif-grid">{visibleGifs.map((gif) => <div className="pro-gif-tile" key={gif.id}><button className="pro-gif-send" onClick={() => selectGif(gif)} aria-label={`Send ${gif.title} GIF`}><span>{gif.emoji}</span><strong>{gif.title}</strong></button><button className={`pro-gif-star ${gifLibrary.favourites.some((item) => item.id === gif.id) ? "active" : ""}`} onClick={() => toggleGifFavourite(gif)} aria-label={`${gifLibrary.favourites.some((item) => item.id === gif.id) ? "Remove" : "Add"} ${gif.title} ${gifLibrary.favourites.some((item) => item.id === gif.id) ? "from" : "to"} favourites`}><Star size={13} /></button></div>)}{!visibleGifs.length && <p className="pro-gif-empty">{gifTab === "all" ? "No GIF matched that search." : `Your ${gifTab} GIFs will appear here.`}</p>}</div></div>}
