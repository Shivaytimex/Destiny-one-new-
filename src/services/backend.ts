...
import type { ChatMessage, CoupleChatSettings, DatePlanStatus, RelationshipReflectionChoice } from '../storage';
...
type ChatMediaKind = 'image' | 'gif' | 'snap' | 'sticker' | 'voice' | 'document';
...
function shouldUploadMediaUri(uri?: string) {
  if (!uri) return false;
  return !/^(https?:|blob:chat-media|chat-media\/|destinyone-gif:)/i.test(uri);
}
...
export async function uploadCurrentUserChatMedia(matchId: string, kind: ChatMediaKind, uri: string) {
  ...
  const userId = await requireCurrentUserId();
  const blob = await uriToBlob(uri);
  const contentType = blob.type || (kind === 'voice' ? 'audio/m4a' : kind === 'gif' ? 'image/gif' : kind === 'document' ? 'application/octet-stream' : 'image/jpeg');
...
export async function fetchMatchMessages(matchId: string, limit = 50) {
  ...
  const {data:stateRows,error:stateError}=await supabase.from('message_user_state').select('*').eq('match_id',matchId).eq('user_id',viewerId);
  if(stateError)throw stateError;
  const states=new Map((stateRows??[]).map(row=>[row.message_id,row]));
  const hydrated=await Promise.all([...(data ?? [])].reverse().map(row => messageRowToChatMessage(row as MessageRow,viewerId)));
  return hydrated.filter(message=>!states.get(message.id)?.hidden).map(message=>{
    const state=states.get(message.id);
    return state?{...message,starredByMe:state.starred,pinnedAt:state.pinned?Date.parse(state.updated_at):undefined,reactions:state.reaction?{me:state.reaction}:undefined}:message;
  });
}
...
async function prepareChatMessageForBackend(matchId: string, message: ChatMessage): Promise<ChatMessage> {
  ...
  if (next.type === 'document' && shouldUploadMediaUri(next.uri)) {
    next.uri = await uploadCurrentUserChatMedia(matchId, 'document', next.uri ?? '') ?? next.uri;
  }
...
export async function messageRowToChatMessage(row: MessageRow,currentUserId?:string): Promise<ChatMessage> {
  ...
  const document = metadata.document && typeof metadata.document === 'object' ? metadata.document as NonNullable<ChatMessage['document']> : undefined;
  ...
  return {
    ...
    mine:currentUserId?row.sender_id===currentUserId:undefined,
    type: row.kind,
    ...
    editedAt: row.edited_at ? Date.parse(row.edited_at) : typeof metadata.editedAt === 'number' ? metadata.editedAt : undefined,
    deletedAt: row.deleted_for_everyone_at ? Date.parse(row.deleted_for_everyone_at) : row.deleted_at ? Date.parse(row.deleted_at) : undefined,
    deletedForEveryone: Boolean(row.deleted_for_everyone_at || metadata.deletedForEveryone),
    forwarded: metadata.forwarded === true,
    linkPreview: metadata.linkPreview && typeof metadata.linkPreview === 'object' ? metadata.linkPreview as ChatMessage['linkPreview'] : undefined,
...
export async function sendMessage(matchId: string, message: ChatMessage) {
  ...
  const metadata = {
    ...
    ...(prepared.editedAt ? { editedAt: prepared.editedAt } : {}),
    ...(prepared.forwarded ? { forwarded: true } : {}),
    ...(prepared.linkPreview ? { linkPreview: prepared.linkPreview } : {}),
  };
  ...
  return messageRowToChatMessage(data as unknown as MessageRow,await requireCurrentUserId());
}

export async function editMatchMessage(matchId:string,messageId:string,text:string){
  ensureBackendConfigured();
  if(!isSupabaseConfigured)return null;
  const {data,error}=await supabase.rpc('edit_match_message',{p_match_id:matchId,p_message_id:messageId,p_body:text.trim()});
  if(error)throw error;
  return messageRowToChatMessage(data as unknown as MessageRow,await requireCurrentUserId());
}

export async function deleteMatchMessage(matchId:string,messageId:string){
  ensureBackendConfigured();
  if(!isSupabaseConfigured)return null;
  const {data,error}=await supabase.rpc('delete_match_message_for_everyone',{p_match_id:matchId,p_message_id:messageId});
  if(error)throw error;
  return messageRowToChatMessage(data as unknown as MessageRow,await requireCurrentUserId());
}

export async function saveMessageUserState(matchId:string,messageId:string,input:{starred?:boolean;pinned?:boolean;hidden?:boolean;reaction?:string|null}){
  ensureBackendConfigured();
  if(!isSupabaseConfigured)return null;
  const {data,error}=await supabase.rpc('set_match_message_user_state',{
    p_match_id:matchId,p_message_id:messageId,p_starred:input.starred??null,p_pinned:input.pinned??null,p_hidden:input.hidden??null,p_reaction:input.reaction??null,p_reaction_changed:Object.prototype.hasOwnProperty.call(input,'reaction'),
  });
  if(error)throw error;
  return data;
}
...
export async function updateDateProposalLifecycle(
  proposalId: string,
  status: Extract<DatePlanStatus, 'cancelled' | 'no_show' | 'unresponsive'>,
) {
  ensureBackendConfigured();
  if (!isSupabaseConfigured) return null;
  const { data, error } = await supabase.rpc('update_date_proposal_lifecycle', {
    p_date_proposal_id: proposalId,
    p_status: status,
  });
  if (error) throw error;
...
export function subscribeToChatMessages(matchId: string, onMessage: (message: ChatMessage) => void): RealtimeChannel | null {
  ...
  void supabase.auth.getUser().then(({data})=>{viewerId=data.user?.id}).catch(()=>undefined);
  return supabase.channel(`postgres-chat:${matchId}`).on(
    'postgres_changes',
    { event: '*', schema: 'public', table: 'messages', filter: `match_id=eq.${matchId}` },
    payload => {
      const row = (payload as unknown as { new?: MessageRow }).new;
      if (!row) return;
      void messageRowToChatMessage(row,viewerId).then(onMessage);
    },
  ).subscribe();
}
...
export async function saveChatSettings(matchId: string, settings: CoupleChatSettings) {
  ensureBackendConfigured();
  if (!isSupabaseConfigured) return null;
  const userId = await requireCurrentUserId();
  const { data, error } = await supabase.from('chat_settings').upsert({
    ...
    conversation_pinned: settings.conversationPinned ?? false,
    conversation_archived: settings.conversationArchived ?? false,
    notification_mode: settings.notificationMode ?? 'all',
    notification_sound: settings.notificationSound ?? 'Destiny Chime',
    muted_until: settings.mutedUntil ? new Date(settings.mutedUntil).toISOString() : null,
    updated_at: new Date().toISOString(),
  }).select().single();
  if (error) throw error;
  return data;
