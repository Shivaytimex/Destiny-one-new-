...
export async function persistMessageEdit(matchId:string,messageId:string,text:string){
  if(!isBackendUuid(matchId)||!isBackendUuid(messageId))return {saved:false,reason:'preview_id'} satisfies PersistenceResult;
  return persistSafely(()=>editMatchMessage(matchId,messageId,text));
}

export async function persistMessageDelete(matchId:string,messageId:string){
  if(!isBackendUuid(matchId)||!isBackendUuid(messageId))return {saved:false,reason:'preview_id'} satisfies PersistenceResult;
  return persistSafely(()=>deleteMatchMessage(matchId,messageId));
}

export async function persistMessageUserState(matchId:string,messageId:string,input:{starred?:boolean;pinned?:boolean;hidden?:boolean;reaction?:string|null}){
  if(!isBackendUuid(matchId)||!isBackendUuid(messageId))return {saved:false,reason:'preview_id'} satisfies PersistenceResult;
  return persistSafely(()=>saveMessageUserState(matchId,messageId,input));
}
...
export async function persistDatePlanStatus(proposalId: string | undefined, status: DatePlanStatus) {
  ...
  if (status === 'cancelled' || status === 'no_show' || status === 'unresponsive') {
    return persistSafely(() => updateDateProposalLifecycle(proposalId, status));
  }
