...
export type MessageRow = {
  ...
  kind: 'text' | 'image' | 'gif' | 'gift' | 'date' | 'snap' | 'sticker' | 'voice' | 'location' | 'document';
  body: string | null;
  ...
  public: {
    Tables: {
      ...
      message_user_state: Table<{
        ...
        updated_at: string;
      }, { match_id: string; message_id: string; user_id: string; starred?: boolean; pinned?: boolean; hidden?: boolean; reaction?: string | null; updated_at?: string }>;
      ...
      gift_orders: Table<{
        ...
        recipient_address_mode: 'recipient_supplied_private' | 'sender_supplied_known_address';
        delivery_window: 'asap' | 'today' | 'scheduled';
        requested_delivery_at: string | null;
        payment_method: 'apple_pay' | 'google_pay' | 'card' | null;
        ...
        inventory_reservation_status: 'pending' | 'reserved' | 'committed' | 'released' | 'expired' | 'unavailable';
        inventory_reserved_until: string | null;
        courier_status: 'not_requested' | 'dispatch_queued' | 'courier_assigned' | 'enroute_pickup' | 'picked_up' | 'enroute_dropoff' | 'delivered' | 'cancelled' | 'failed';
        courier_display_name: string | null;
        courier_vehicle_summary: string | null;
        proof_status: 'not_available' | 'pending' | 'verified' | 'failed';
      ...
      gift_order_items: Table<{
        id: string; gift_order_id: string; product_id: string; product_name: string; quantity: number;
        unit_price_cents: number; line_total_cents: number; created_at: string;
      }>;
      gift_order_checkout_private: Table<{
        gift_order_id: string; payment_method: 'apple_pay' | 'google_pay' | 'card';
        payment_method_reference: string | null; confirmation_email: string | null; created_at: string; updated_at: string;
      }>;
      gift_catalog_products: Table<{
        id:string;sku:string;name:string;category:string;occasion:string;description:string;emoji:string;photo_url:string;
        price_cents:number;currency:'USD'|'CAD'|'INR';service_level:'on_demand'|'same_day'|'scheduled';delivery_fee_cents:number;prep_minutes:number;
        travel_minutes:number;window_buffer_minutes:number;cutoff_hour:number;pickup_partner_name:string;active:boolean;catalog_version:string;updated_at:string;
      }>;
      gift_inventory: Table<{id:string;product_id:string;merchant_reference:string;country_code:'US'|'CA'|'IN';city:string;units_on_hand:number;units_reserved:number;status:'available'|'paused'|'sold_out';provider_version:string|null;synced_at:string}>;
      gift_inventory_reservations: Table<{id:string;gift_order_id:string;inventory_id:string;product_id:string;quantity:number;status:'reserved'|'committed'|'released'|'expired';expires_at:string;committed_at:string|null;released_at:string|null;created_at:string}>;
      gift_delivery_proofs: Table<{id:string;gift_order_id:string;provider:string;provider_event_id:string;proof_type:'photo'|'pin'|'signature'|'barcode'|'recipient_hand_off';proof_reference:string|null;verified_at:string;retention_expires_at:string;created_at:string}>;
      gift_merchants: Table<{id:string;external_reference:string;display_name:string;legal_name:string|null;country_code:'US'|'CA'|'IN';supported_cities:string[];timezone:string;status:'onboarding'|'active'|'paused'|'suspended';verification_status:'pending'|'verified'|'rejected';fulfillment_score:number;support_email:string|null;provider_adapter:string;last_catalog_sync_at:string|null;last_inventory_sync_at:string|null;created_at:string;updated_at:string}>;
      gift_merchant_catalog_items: Table<{id:string;merchant_id:string;product_id:string;merchant_sku:string;title_override:string|null;image_url_override:string|null;unit_price_minor:number;currency:'USD'|'CAD'|'INR';available_for_substitution:boolean;active:boolean;provider_version:string|null;synced_at:string}>;
      gift_market_prices: Table<{product_id:string;country_code:'US'|'CA'|'IN';currency:'USD'|'CAD'|'INR';unit_price_minor:number;base_delivery_fee_minor:number;tax_rate_bps:number;minimum_order_minor:number;active:boolean;pricing_version:string;updated_at:string}>;
      gift_city_coverage: Table<{country_code:'US'|'CA'|'IN';city_key:string;city_label:string;currency:'USD'|'CAD'|'INR';timezone:string;courier_provider:string;on_demand_enabled:boolean;same_day_enabled:boolean;scheduled_enabled:boolean;service_radius_km:number;status:'pilot'|'active'|'waitlist'|'paused';updated_at:string}>;
      gift_substitution_preferences: Table<{gift_order_id:string;member_id:string;allow_substitution:boolean;max_price_increase_minor:number;allowed_categories:string[];excluded_product_ids:string[];updated_at:string}>;
      gift_substitution_offers: Table<{id:string;gift_order_id:string;original_product_id:string;replacement_product_id:string;price_delta_minor:number;currency:'USD'|'CAD'|'INR';reason:string;status:'pending'|'accepted'|'declined'|'expired';expires_at:string;responded_at:string|null;created_at:string}>;
      gift_order_issues: Table<{id:string;gift_order_id:string;opened_by:string;issue_type:'failed_delivery'|'damaged_item'|'wrong_item'|'missing_item'|'late_delivery'|'payment'|'other';description:string;requested_resolution:'redelivery'|'refund'|'replacement'|'review';status:'open'|'investigating'|'awaiting_evidence'|'resolved'|'declined';priority:'normal'|'high'|'urgent';assigned_to:string|null;resolution_summary:string|null;resolved_at:string|null;created_at:string;updated_at:string}>;
      gift_notification_jobs: Table<{id:string;gift_order_id:string|null;audience:'sender'|'recipient'|'support'|'merchant';channel:'in_app'|'email'|'push'|'sms';template_key:string;destination_token:string|null;payload:Json;idempotency_key:string;status:'queued'|'processing'|'sent'|'retry'|'failed'|'cancelled';attempt_count:number;max_attempts:number;next_attempt_at:string;locked_at:string|null;locked_by:string|null;provider_message_id:string|null;last_error_code:string|null;sent_at:string|null;created_at:string;updated_at:string}>;
      gift_provider_jobs: Table<{id:string;gift_order_id:string|null;provider:string;job_type:'catalog_sync'|'inventory_sync'|'inventory_reserve'|'payment_authorize'|'merchant_submit'|'courier_dispatch'|'refund'|'proof_retention';idempotency_key:string;payload:Json;status:'queued'|'processing'|'succeeded'|'retry'|'failed'|'dead_letter';attempt_count:number;max_attempts:number;next_attempt_at:string;locked_at:string|null;locked_by:string|null;last_error_code:string|null;last_error_summary:string|null;completed_at:string|null;created_at:string;updated_at:string}>;
      gift_fraud_assessments: Table<{id:string;gift_order_id:string|null;actor_id:string|null;risk_score:number;decision:'approve'|'review'|'block';reason_codes:string[];velocity_bucket:string;provider_reference:string|null;created_at:string}>;
      gift_recommendation_feedback: Table<{id:string;member_id:string;product_id:string;context_key:string;signal:'viewed'|'added'|'removed'|'purchased'|'substituted'|'liked'|'disliked'|'delivered_positive'|'delivered_negative';anonymous_features:Json;created_at:string}>;
      gift_commerce_alerts: Table<{id:string;alert_key:string;severity:'info'|'warning'|'critical';category:'inventory'|'merchant'|'courier'|'payment'|'notification'|'fraud'|'order';title:string;summary:string;status:'open'|'acknowledged'|'resolved';first_seen_at:string;last_seen_at:string;acknowledged_by:string|null;resolved_at:string|null}>;
      ...
      chat_settings: Table<{
        ...
        conversation_pinned: boolean;
        conversation_archived: boolean;
        notification_mode: 'all' | 'mentions' | 'muted';
        notification_sound: 'Destiny Chime' | 'Soft Rose' | 'Classic' | 'Silent';
        muted_until: string | null;
        updated_at: string;
      }, { match_id: string; user_id: string; nickname?: string | null; theme?: string; retention_mode?: 'keep' | 'after_seen' | '24_hours' | '7_days'; screenshot_alerts?: boolean; conversation_pinned?: boolean; conversation_archived?: boolean; notification_mode?: 'all' | 'mentions' | 'muted'; notification_sound?: 'Destiny Chime' | 'Soft Rose' | 'Classic' | 'Silent'; muted_until?: string | null; updated_at?: string }>;
      ...
      call_sessions: Table<{
        id: string; client_call_id: string; match_id: string; caller_id: string; call_type: 'audio' | 'video';
        status: 'ringing' | 'accepted' | 'rejected' | 'ended' | 'missed' | 'failed'; started_at: string;
        answered_at: string | null; ended_at: string | null; failure_reason: string | null;
      }>;
      ...
      billing_verification_attempts: Table<{
        id: string; user_id: string; purchase_session_id: string | null; restore_session_id: string | null;
        platform: 'apple_iap' | 'google_play'; product_key: string | null; purchase_token_hash: string;
        transaction_hash: string | null; status: 'pending' | 'provider_unavailable' | 'rejected' | 'verified' | 'ledger_failed';
        attempt_count: number; error_code: string | null; provider_event_id_hash: string | null;
        verified_at: string | null; created_at: string; updated_at: string;
      }>;
      launch_analytics_sessions: Table<{
        id: string; user_id: string; platform: 'ios' | 'android' | 'web'; app_version: string;
        build_variant: 'development' | 'pilot' | 'production' | 'preview'; event_count: number;
        started_at: string; last_seen_at: string; ended_at: string | null;
      }>;
      launch_analytics_events: Table<{
        id: string; session_id: string; user_id: string; event_name: string; properties: Json;
    ...
    Functions: {
      respond_gift_order: { Args: { p_order_id: string; p_accept: boolean; p_dropoff?: Json | null }; Returns: Json };
      reserve_gift_inventory_for_order: { Args: { p_order_id:string;p_hold_minutes?:number }; Returns: Json };
      release_gift_inventory_for_order: { Args: { p_order_id:string;p_reason?:string }; Returns: boolean };
      commit_gift_inventory_for_order: { Args: { p_order_id:string }; Returns: boolean };
      expire_gift_inventory_reservations: { Args: { p_limit?:number }; Returns: number };
      record_gift_delivery_proof: { Args: { p_order_id:string;p_provider:string;p_provider_event_id:string;p_proof_type:string;p_proof_reference:string|null;p_verified_at:string }; Returns: boolean };
      process_gift_delivery_webhook_v2: { Args: { p_provider:string;p_provider_event_id:string;p_external_delivery_id:string;p_event_type:string;p_tracking_url:string|null;p_payload_hash:string;p_courier_display_name?:string|null;p_vehicle_summary?:string|null;p_occurred_at?:string }; Returns: Json };
      get_gift_marketplace_v4: { Args: { p_country_code:string;p_city:string;p_currency:string;p_query?:string;p_sort?:string;p_service_level?:string|null }; Returns: Json };
      respond_gift_substitution_offer_v4: { Args: { p_offer_id:string;p_accept:boolean }; Returns: Json };
      claim_gift_provider_jobs_v4: { Args: { p_worker:string;p_limit?:number }; Returns: Json };
      finish_gift_provider_job_v4: { Args: { p_job_id:string;p_success:boolean;p_error_code?:string|null;p_error_summary?:string|null }; Returns: boolean };
      claim_gift_notification_jobs_v4: { Args: { p_worker:string;p_limit?:number }; Returns: Json };
      finish_gift_notification_job_v4: { Args: { p_job_id:string;p_success:boolean;p_provider_message_id?:string|null;p_error_code?:string|null }; Returns: boolean };
      refresh_gift_commerce_alerts_v4: { Args: Record<string,never>; Returns: number };
      daily_matches: {
      ...
      update_date_proposal_lifecycle: { Args: { p_date_proposal_id: string; p_status: 'cancelled' | 'no_show' | 'unresponsive' }; Returns: Json };
      ...
      edit_match_message: {
        Args: { p_match_id: string; p_message_id: string; p_body: string };
        Returns: Json;
      };
      delete_match_message_for_everyone: {
        Args: { p_match_id: string; p_message_id: string };
        Returns: Json;
      };
      set_match_message_user_state: {
        Args: { p_match_id: string; p_message_id: string; p_starred?: boolean | null; p_pinned?: boolean | null; p_hidden?: boolean | null; p_reaction?: string | null; p_reaction_changed?: boolean };
        Returns: Json;
      };
      join_city_waitlist: {
      ...
      start_launch_analytics_session: {
        Args: { p_session_id: string; p_platform: 'ios' | 'android' | 'web'; p_app_version: string; p_build_variant: 'development' | 'pilot' | 'production' | 'preview' };
        Returns: boolean;
      };
      record_launch_analytics_event: {
        Args: { p_event_id: string; p_session_id: string; p_event_name: string; p_properties?: Json; p_occurred_at?: string };
        Returns: boolean;
      };
      close_launch_analytics_session: { Args: { p_session_id: string }; Returns: boolean };
      get_launch_analytics_snapshot: { Args: { p_since?: string }; Returns: Json };
      ...
      mark_match_messages_delivered: { Args: { p_match_id: string }; Returns: number };
      mark_match_messages_read: { Args: { p_match_id: string }; Returns: number };
      start_match_call: { Args: { p_match_id: string; p_client_call_id: string; p_call_type: 'audio' | 'video' }; Returns: Json };
      update_match_call: { Args: { p_match_id: string; p_client_call_id: string; p_status: 'accepted' | 'rejected' | 'ended' | 'missed' | 'failed'; p_failure_reason: string | null }; Returns: Json };
    };
