...
const serviceSource = [
  ...
  'src/services/launchAnalytics.ts',
...
const edgeSource = [
  ...
  'supabase/functions/reserve-gift-inventory/index.ts',
  'supabase/functions/gift-delivery-webhook/index.ts',
].map((file) => readFileSync(file, 'utf8')).join('\n');
...
describe('complete backend deployment contract', () => {
  ...
  it('keeps a unique, explicitly versioned production inventory', () => {
    ...
    expect(contractSource).toContain("id: 'destinyone-backend-v38'");
    expect(contractSource).toContain('schemaVersion: 38');
    expect(new Set(tables).size).toBe(tables.length);
    expect(new Set(rpcs).size).toBe(rpcs.length);
    expect(tables.length).toBe(117);
    expect(rpcs.length).toBe(105);
  });
});
