...
describe('member data runtime wiring', () => {
  ...
  it('keeps invented chat, likes, and presence signals in preview-only surfaces', () => {
    expect(app).toContain("const isChatPreview=memberDataRuntime.source==='preview'||isPreviewAccessMode;");
    expect(app).toContain("!isCoupleMode&&isChatPreview&&<><View style={styles.iceReveal}");
    expect(app).toContain("!isCoupleMode&&partnerTyping&&<View accessible accessibilityLabel={`${match.name} is typing`}");
    expect(app).toContain("if(memberDataRuntime.source==='preview'||isPreviewAccessMode){");
