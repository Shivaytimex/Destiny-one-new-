...
`frontendRepository.ts` is a browser-only preview fixture. Its methods are named around product actions (`saveProposal`, `saveDecision`, `send`) rather than HTTP verbs. This prevents REST and database details from leaking into UI components.
