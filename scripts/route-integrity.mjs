...
function pageRoute(path) {
  let route = `/${relative(pagesRoot, path).split(sep).join("/").replace(/\.(?:jsx|tsx)$/, "")}`;
  route = route.replace(/\/index$/, "/");
  return route === "/index.actual" || route === "/_app" ? null : route;
}

const routes = files(pagesRoot).filter((path) => [".jsx", ".tsx"].includes(extname(path))).map(pageRoute).filter(Boolean);
const patterns = routes.map((route) => new RegExp(`^${route.replace(/\[([^\]]+)\]/g, "[^/]+").replace(/\/$/, "")}/?$`));
const hrefs = new Set();

for (const path of files(root).filter((item) => [".js", ".jsx", ".ts", ".tsx"].includes(extname(item)))) {
